...
The application consists of **6 main pages** that create a complete game time tracking experience:
...
| 📊 **User Stats Dashboard** | `/stats/:userId` | Personal statistics      | Time bars, percentages, total time, action buttons      |
| 🏆 **Global Statistics**    | `/leaderboard`   | Comparative analytics    | Charts, leaderboards, global trends                     |
...
2. Users Hub → Select or switch between players
3. Games Library → Choose from 4 retro games
4. Game Session → Play and track time
5. User Stats Dashboard → View personal progress
6. Global Statistics → Compare with others
```
...
interface User {
  ...
  firstName: string
  lastName: string
  profilePicture?: string // File path to uploaded image or default silhouette
  createdAt: Date
  updatedAt: Date
}
```
...
interface Game {
  ...
  description: string;
  icon: string; // Retro game icon/sprite
...
```
mongo-gameapp/
├── client/                 # React frontend (Person 2)
│   ├── src/
...
│   │   ├── api/            # API calls
│   │   ├── assets/         # Images, icons, default avatars
│   │   └── App.tsx
│   ├── package.json
│   └── vite.config.ts
├── server/                 # Node.js backend (Person 1)
│   ├── src/
...
1. User selects from 4 retro games (Pac-Man, Tetris, Space Invaders, Asteroids)
2. Click "Start" to begin timer for selected game
3. Timer runs and displays current session time
4. Click "Stop" to end session
5. Calculate duration and store in database
6. Update user statistics

### 3. Statistics Flow

1. Fetch user's game sessions
2. Calculate total time per game
3. Generate comparative statistics
4. Display charts and leaderboards
...
### 🤔 **Axios vs Prisma - What's the Difference?**

**Axios** ≠ **Prisma** - They're completely different tools:
...
- **Axios**: HTTP client library for making API requests
  - Used on the **frontend** to call your backend APIs
  - Alternative to the built-in `fetch` function

### 💡 **Why Skip Axios? Use Native Fetch Instead!**

**Benefits of using `fetch` (built-in):**
...
✅ **Less to learn** - One less library to master
...
- **Key Features**: User grid display, "Add User" button, weather widget
- **Figma Reference**: Grid of user avatars page

**3. Games Library** (`/games`)

- **Purpose**: Browse and select from 4 retro games
- **Key Features**: Game selection grid, "Choose a game to play" prompt
- **Figma Reference**: 4 retro game icons page

**4. Game Session** (`/play/:gameId`)
...
- **Purpose**: Personal statistics and game time analysis
- **Key Features**: Time bars, percentages, total time, action buttons
- **Figma Reference**: User profile with detailed statistics

**6. Global Statistics** (`/leaderboard`)

- **Purpose**: Comparative stats, charts, and leaderboards
- **Key Features**: Charts, trends, global rankings, weather widget
- **Figma Reference**: Charts and leaderboard page
...
- `UserRegistrationForm` - With optional file upload
...
- `Timer` - Digital timer with retro styling ⏱️
- `SessionControls` - Start/Stop buttons ▶️⏹️
- `StatisticsChart` - Using Recharts for data visualization 📊
- `Leaderboard` - Retro-styled ranking table 🏆
...
## 📊 Statistics & UI Requirements (Based on Figma Design)
...
- User selection dropdown/navigation

**2. Game Time Bars (Minutes Played):**

- Horizontal bar chart showing time played per game
- Scale: 0-100+ minutes (bars extend based on actual time)
- Format: "Game 1: 40 min", "Game 2: 53 min", etc.
- Visual bars with different fill levels

**3. Game Percentage Chart:**

- Circular progress indicators OR percentage display
- Shows percentage of total time spent on each game
- Format: "Game 1: 40%", "Game 2: 53%", etc.
- 4 game entries with individual percentages

**4. Total Time Display:**

- Large prominent number: "164 min"
- Label: "Total time played"
- Central position in the layout
